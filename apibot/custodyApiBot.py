...
def custodyLoginGet2FA(hostUrl, userName, password, isAdmin = False):
    ...
    # print("sha256", sha256(password).hexdigest())
    rsp = apiPost(hostUrl, api, {"password": sha256(password).hexdigest(), "mail": userName, "isAdmin": isAdmin})
    if rsp != None:
        if rsp["respCode"] == 0:
...
mail = config.mailOfCustodyuser
mailpass = config.mailpassOfCustodyuser
mailid = config.mailidOfCustodyuser
isAdmin = config.isAdminOfCustodyuser
gasecret = config.gasecretUATOfCustodyuser

custodyAccount = config.custodyAccountOfCustodyuser
custodyPass = config.custodyPassOfCustodyuser
...
try:
    ...
    if custodyLoginGet2FA(custodyUrl, custodyAccount, custodyPass):
        t2 = time.time()
        for i in range(20):
            print("try "+str(i)+" time")
            time.sleep(10)
            code = get2FACode(mail, mailpass, int(t2))
            if code == '':
                continue
            else:
                print("2FA:", code)
